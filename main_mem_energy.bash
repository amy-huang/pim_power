...
find_stat () {
    for ctrl_num in {0..7}
    do
	    stat=$(grep timestamp$1 $2 | grep $3$ctrl_num | grep $4 | awk '{s=$2}END{printf ("%f", s)}')
	    if [ "$stat" == "nan" ] || [ "$stat" == "" ]; then
	        stat=0
	    fi
	    echo "timestamp $1 in $2 for $3$ctrl_num $4: $stat"
    done
}
...
do
    ...
    find_stat $num_threads pim-stats.txt pim_vault_ctrls readRowHitRate
    find_stat $num_threads pim-stats.txt mem_ctrls readRowHitRate

#	for ctrl_num in {0..7}
#	do

        #find_stat $num_threads host-stats.txt pim_vault_ctrls readRowHitRate
        #find_stat $num_threads host-stats.txt mem_ctrls readRowHitRate

        #find_stat $num_threads pim-stats.txt pim_vault_ctrls num_reads::total
        #find_stat $num_threads pim-stats.txt mem_ctrls num_reads::total

        #find_stat $num_threads host-stats.txt pim_vault_ctrls num_reads::total
        #find_stat $num_threads host-stats.txt mem_ctrls num_reads::total
